...
/*
 * file: embedded_noalloc.c
 * description: demonstrates the zero-allocation parsing mode (cjsonx_parse_with_buffer),
 * which is ideal for embedded systems with strict memory constraints.
 */
...
int main() {
    ...
    /* freeing is safe (and effectively a no-op since it's static) */
    cjsonx_doc_free(doc);
